...
# The handwritten QMD tree is canonical; rendered output lives in OUTPUT_DIR only.
...
# Clean up staging and output artifacts only — never the source tree
clean_up() {
...
# Generate one SKILL.md wrapper per public stack area in the source tree
generate_skills() {
	echo "Generating skill wrappers..."
	if [[ ! -d "$ARTICLES_DIR" ]]; then
		echo "Articles directory '$ARTICLES_DIR' not found. Skipping skill wrappers."
		return 0
	fi
	local count=0
	local area_dir lang area qmd title public_qmds
	for area_dir in "$ARTICLES_DIR"/*/*/; do
		[[ -d "$area_dir" ]] || continue
		public_qmds=$(fd "^[^_].*\.qmd$" "$area_dir" --max-depth 1 2>/dev/null || true)
		[[ -z "$public_qmds" ]] && continue
		lang=$(basename "$(dirname "$area_dir")")
		area=$(basename "$area_dir")
		{
			echo "---"
			echo "name: articles-$lang-$area"
			echo "description: Handmade QMD articles covering $lang/$area concepts and patterns."
			echo "---"
			echo ""
			echo "<!-- generated by build.sh generate_skills; safe to regenerate; do not hand-edit -->"
			echo ""
			echo "Handwritten articles by Andres Monge. Read the .qmd files in this"
			echo "directory directly when working with $lang/$area topics:"
			echo ""
			while IFS= read -r qmd; do
				title=$(sed -n 's/^title:[[:space:]]*"\{0,1\}\(.*\)$/\1/p' "$qmd" | head -1)
				title="${title%\"}"
				echo "- $(basename "$qmd")${title:+ — $title}"
			done <<<"$public_qmds"
		} >"$area_dir/SKILL.md"
		count=$((count + 1))
	done
	echo "Skill wrappers generated: $count"
}
